...
# 3) Compute streak lengths for the observed data
...
# Flatten the streaks into a single column for analysis
streaks_unnested <- streaks_df %>%
  unnest(cols = streaks_list) %>%
  rename(streak_length = streaks_list)

# Summarize the streak lengths
summary(streaks_unnested$streak_length)

# 4) Visualize streak length distribution for observed data
library(ggplot2)

ggplot(streaks_unnested, aes(x = streak_length)) +
  geom_histogram(binwidth = 1, color = "black", fill = "steelblue") +
  labs(
    title = "Distribution of Streak Lengths (Observed Data)",
    x = "Streak Length",
    y = "Count"
  ) +
  theme_minimal()
...
# 5) Simulate streaks assuming p = 0.508
...
# Compute streak lengths for simulated data
sim_streaks <- compute_streaks(sim_df$same_side)

# Create dataframe for simulated streaks
sim_streaks_df <- data.frame(streak_length = sim_streaks)

# 6) Visualize simulated streak lengths
ggplot(sim_streaks_df, aes(x = streak_length)) +
  geom_histogram(binwidth = 1, color = "black", fill = "lightgreen") +
  labs(
    title = "Distribution of Streak Lengths (Simulated Data, p = 0.508)",
    x = "Streak Length",
    y = "Count"
  ) +
  theme_minimal()
...
# 7) Chi-Square Test: Compare observed vs. simulated streak distributions

# Bin the data into categories to avoid sparse counts
obs_freq <- table(cut(streaks_unnested$streak_length, breaks = c(0, 1, 2, 3, 4, 5, 10, Inf)))
sim_freq <- table(cut(sim_streaks_df$streak_length, breaks = c(0, 1, 2, 3, 4, 5, 10, Inf)))

# Display observed and simulated frequencies
obs_freq
sim_freq

# Chi-square test to compare observed and simulated distributions
chisq_test <- chisq.test(obs_freq, p = prop.table(sim_freq))
